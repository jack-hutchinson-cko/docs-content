...
*   Created by [Chrisi Webster](    /display/~chrisiwebster
    ), last modified on [Mar 16, 2021](/pages/diffpagesbyversion.action?pageId=20120924&selectedPageVersions=1&selectedPageVersions=2 "Show changes")
...
For single iframes, Frames's JavaScript library will dynamically apply an `--invalid` modifier to the container element class.
...
For multiple iframes, Frames's JavaScript library will dynamically apply an `--invalid` modifier to **each** container element class.
...
[Add Comment](/display/x/Frames+customization+guide?showComments=true&showCommentArea=true#addcomment)
