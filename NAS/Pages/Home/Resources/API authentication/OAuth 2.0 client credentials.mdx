...
*   Created by [Toby Knott](    /display/~toby.knott
    ), last modified on [Apr 29, 2021](/pages/diffpagesbyversion.action?pageId=20121049&selectedPageVersions=14&selectedPageVersions=15 "Show changes")
...
1.  Make a request to our /tokens endpoint, exchanging your access key ID and secret for an access token. As part of this request, you can also include one or more scopes, which define which endpoints the token can access.
2.  If your request is successful, the response will contain an access token.
3.  Use the newly created access token to authenticate requests to our APIs.
...
[Add Comment](/display/x/OAuth+2.0+client+credentials?showComments=true&showCommentArea=true#addcomment)