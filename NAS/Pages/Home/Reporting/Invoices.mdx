...
*   Created by user-faddb, last modified by [Toby Knott](    /display/~toby.knott
    ) on [Apr 12, 2021](/pages/diffpagesbyversion.action?pageId=8488094&selectedPageVersions=6&selectedPageVersions=7 "Show changes")
...
You will receive invoices for each of your active entities and each invoice will be in a given holding currency. This means that for each entity you have set up with us, you will receive _n_ number of invoices for _n_ number of holding currencies you have set up.
...
These pages contain tables showing how the totals on the summary page are calculated. For acquiring transactions, these calculated amounts are split by payment method.
 ...
 Learn about the fee categories and the fees comprising them
...
We offer both **gross** and **net** settlement. The accompanying [payouts](/display/x/Payouts+reports) and [financial actions reports by payout ID](/display/x/Financial+actions+reports) contain a detailed calculation and justification.
...
*   For **gross settlement** all fees are invoiced to you for the previous month’s processing and no fees are deducted from your settlements in that period. On your invoice the _total amount due_ may be positive or negative. When making a payment to settle your invoice, please include the _invoice no._ in the payment reference so we can identify the related invoice to settle.
    

If you opt for **gross settlement**, you can track the outstanding fees total intra-month using our [balance reports](/display/x/Balance+reports), and view the detailed fees applied using the [financial actions by date range reports](/display/x/Financial+actions+reports).
...
If you opted for **net settlement**, [click here to download a sample invoice](https://assets.checkout.com/docs/downloads/sample_invoice_net.pdf).

If you opted for **gross settlement**, [click here to download a sample invoice](https://assets.checkout.com/docs/downloads/sample_invoice.pdf).

If you're using Safari, hold the **Option** key when you click to download.
...
[Add Comment](/display/x/Invoices?showComments=true&showCommentArea=true#addcomment)
