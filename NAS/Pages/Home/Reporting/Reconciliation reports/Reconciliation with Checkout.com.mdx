...
*   Created by user-faddb, last modified by [Toby Knott](    /display/~toby.knott
    ) on [Apr 26, 2021](/pages/diffpagesbyversion.action?pageId=8488908&selectedPageVersions=5&selectedPageVersions=6 "Show changes")
...
![](/download/attachments/8488908/Payouts%20Report.PNG?version=1&modificationDate=1619432521088&api=v2)
...
![](/download/attachments/8488908/Financial%20Actions.PNG?version=1&modificationDate=1619433109093&api=v2)
...
![](/download/attachments/8488908/Balance%20report%20highlights.PNG?version=1&modificationDate=1619433932920&api=v2)
...
![](/download/attachments/8488908/Balance%20Report%20Recon.PNG?version=1&modificationDate=1619434194902&api=v2)
...
Information on the mapping between Action Type, Breakdown Type, and the fee category fields on the balance report is available [here](/display/x/Breakdown+types).
...
1.  First, using the financial actions report, filter by the **Currency Account ID**.
    
2.  Then filter by the **Payment Method** if applicable.
    
3.  Then filter by **Action Type** to include only **authentication**, **authorization**, **capture,** and **void** actions.
    
4.  Then filter on **Breakdown Type** filtered to include only **Scheme Fixed Fees** and **Scheme Variable Fees**. The sum of the **Holding Currency** column will reconcile with the **Card Payment Scheme Fees** displayed on your payouts report.
...
[Add Comment](/display/x/Reconciliation+with+Checkout.com?showComments=true&showCommentArea=true#addcomment)