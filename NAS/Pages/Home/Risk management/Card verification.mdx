...
*   Created by user-faddb, last modified by [Jack Hutchinson](    /display/~jack.hutchinson
    ) on [Mar 22, 2021](/pages/diffpagesbyversion.action?pageId=8488776&selectedPageVersions=2&selectedPageVersions=3 "Show changes")
...
To verify a card, simply [set up a payment request](/display/x/Request+a+payment) with the `amount` field omitted or set to `0`.
...
[Add Comment](/display/x/Card+verification?showComments=true&showCommentArea=true#addcomment)
