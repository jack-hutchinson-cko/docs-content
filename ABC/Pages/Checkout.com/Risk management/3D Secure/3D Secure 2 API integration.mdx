...
*   Created by [Toby Knott](    /display/~toby.knott
    ), last modified on [May 24, 2021](/pages/diffpagesbyversion.action?pageId=2785691&selectedPageVersions=18&selectedPageVersions=19 "Show changes")
...
First, the customer needs to exchange their card details for a card token – tokenization. You can create a card token with one of our [integration options](/display/DOCS/Integrate).

#### Step 2: Create a card token payment request

Once you've got a card token, you can request a payment using [the request a card payment endpoint](/display/DOCS/Request+a+card+payment).
...
If you want to test 3DS authentication flows and transaction statuses in the sandbox environment, [use our test cards](/display/DOCS/3D+Secure+testing).
...
![](/download/attachments/2785691/3ds2_challenge.png?version=1&modificationDate=1586863846280&api=v2)
...
Once you've got the `cko-session-id`, you can use our [get payment details endpoint](/display/DOCS/Get+payment+details) to determine whether the payment was authenticated and authorized.

The `cko-session-id` expires 15 minutes after being created.

#### Endpoints

*   [**Live**](#tabs-188d4be0-f2ff-43ba-a26a-96bbd53ae2ae-1)
*   [**Sandbox**](#tabs-188d4be0-f2ff-43ba-a26a-96bbd53ae2ae-2)
...
![](/download/attachments/2785691/Non-3DS%20authorization.png?version=1&modificationDate=1619705981902&api=v2)
...
![](/download/attachments/2785691/Non-3DS%20authorization%20with%20soft%20decline.png?version=1&modificationDate=1619705996050&api=v2)
...
[Add Comment](/display/DOCS/3D+Secure+2+API+integration?showComments=true&showCommentArea=true#addcomment)