...
. you are processing payments from cards issued in Europe (including the UK), **and** 2. you are processing payments through a European (including the UK) acquirer.
...
DS authentication either failed or could not be attempted.

DS1 and 2

## Mastercard liability shift rules

lectronic Commerce Indicator
...
DS authentication was successful; transaction secured by 3DS.

DS1 and 2

rictionless authentication via the Mastercard Identity Check Data Only service.

DS2

ransaction is [out of scope](#id-3DSecure-outofscope) or [exempt](#id-3DSecure-exemptions) from Strong Customer Authentication.
...
e're gradually switching European merchants' traffic over to 3DS2 in countries where card issuers widely support the new protocol. If they don't yet support the new version, the request will automatically default to the original 3DS.
