...
*   Created by [Toby Knott](    /display/~toby.knott
    ...
    ) on [Apr 30, 2021](/pages/diffpagesbyversion.action?pageId=5931062&selectedPageVersions=21&selectedPageVersions=22 "Show changes")
...
2.  Enter the following URL, replacing `example.com` with the URL of your shop: `example.com/checkout_com/webhook/callback`. (A webhook URL specific to your installation is available in the _Account Settings_ section of the plugin configuration panel.)
...
5.  Click on the webhook you just created, and make a note of the private shared API key.  
...
1.  [Purchase the extension from Magento Marketplace](https://marketplace.magento.com/checkoutcom-magento2.html) (it's free).
...
1.  Log in to you Magento Admin Panel and go to _Stores > Configuration > Sales > Payment Methods_.
2.  Find the Checkout.com plugin and click **Configure**.
3.  In this configuration panel, go to _Configuration > Global Settings_.
4.  Make sure _Environment_ is set to **Test** so you can start testing card payments.  
    ![](/download/attachments/5931062/Magento_2_plugin_global_settings.png?version=1&modificationDate=1587400855363&api=v2)
5.  Select the _Payment Action_. **Authorize and Capture** means that the charge is authorized and captured (i.e., the money due from the customer's account is moved into your account) at the same time. **Authorize** will only authorize the charge; you have to manually capture the payment yourself. If you're unsure, **Authorize and Capture** is likely the right choice.
6.  Then go to _Account Settings_.
7.  Enter your **Secret Key** and **Public Key**. If you're testing, make sure you use the API keys from your sandbox Hub account, then click _Set Webhooks_ to automatically register your webhooks in [Checkout.com](http://Checkout.com) Hub.  
...
8.  Go to _Sales > Orders_. Your test order will be displayed there with a `Processing` status (or `Pending Paymen`t if you choose the **Authorize only** payment action). This indicates that the payment has been successfully captured. The transaction will also appear in the _Payments_ section of your sandbox Hub account.
...
[Add Comment](/display/DOCS/Magento+2?showComments=true&showCommentArea=true#addcomment)