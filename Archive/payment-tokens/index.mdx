...
| `id` <span class="field-type">String</span> | The unique identifier of a payment token (prefixed with pay*tok*). |
| `livemode` <span class="field-type">Boolean</span> | - `true` if live keys were used in the request. \* `false` if test keys were used in the request. |
