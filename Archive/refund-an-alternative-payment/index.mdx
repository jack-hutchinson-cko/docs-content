---
title: refund-an-alternative-payment
account: Archive
---

*   Header
*   Code Sample
*   Callout
*   Table
*   Image
*   Embed
*   HTML/CSS

View page on sitePage history[Suggested Edits 0](/project/checkoutdocs-legacy/v1.9/suggested)

Use this API endpoint to refund an alternative payment. The method for refunding alternative payments is the same no matter which payment type you used. 

\## The request Capture a payment by sending a \*\*POST\*\* API request to the URI below. In the URI, replace \*\*{chargeId}\*\* with the \`chargeId\` of the \*\*authorized transaction\*\*, however, first \*\*add the prefix\*\* \`charge\_\` or \`charge\_test\_\` to your Id. ### Endpoint live <pre class="api-header-long-url"><p class="api-header"><span class="post-tag">POST</span>https://api2.checkout.com/v2/charges/charge\_{chargeId}/refund</p></pre> ### Endpoint sandbox <pre class="api-header-long-url"><p class="api-header"><span class="post-tag">POST</span>https://sandbox.checkout.com/api2/v2/charges/charge\_{chargeId}/refund</p></pre> ### Header and path parameters 

 x     

Add Row Add Col

|  |  |
| --- | --- |
| 
\`Authorization\` <span class="req-def">Required</span> 

 | 

\`secret key\`\* 

 |
| 

\`Content-Type\` <span class="req-def">Required</span> 

 | 

\`application/json\` 

 |

\*For authorization, use the valid \`secret key\` of your Checkout.com account. You can find this in \[The Hub\](https://hub.checkout.com/). 

 x     

Add Row Add Col

|  |  |
| --- | --- |
| 
\`chargeId\` <span class="req-def">Required</span> 

 | 

The charge Id, embedded within the URI, prefixed with \`charge\_\` or \`charge\_test\_\`. 

 |

\### Request example 

Shell

rename · delete

TextASPCCoffeeScriptClojureC++C#CSScURLCypherDDiffDockerfileErlangGoGroovyHandlebarsHAMLHaxeHTMLHTTPJavaJavaScriptJinja2JSONJuliaKotlinLESSLiquidLuaMarkdownMySQLObjective-COCamlPerlPHPPostgresPowerShellPythonRRubyRustSASSScalaSCSSShellSmartySQLStylusSwiftTOMLTwigTypeScriptXMLYAML

xxxxxxxxxx

1

curl https://api2.checkout.com/v2/charges/charge\_30FDCABC175J76BD3E18/refund

2

 \-H "Authorization: sk\_test\_55aedccc-7f53-4ccc-b0a6-d943decc3c31"

3

 \-H "Content-Type:application/json;charset=UTF-8"

4

 \-X POST

Import from Gist

\## The response If the refund was successful, the status of the of the payment should show in the response as \*Refunded\*. The \`id\` listed in the first line of the response is a unique ID linked to the latest status change of the charge. The \`originalId\` listed in the second line of the response is the unique ID of the previous status change. If you're having problems using this API, contact our \[support team\](mailto:support@checkout.com) for help. 

JSON

rename · delete

TextASPCCoffeeScriptClojureC++C#CSScURLCypherDDiffDockerfileErlangGoGroovyHandlebarsHAMLHaxeHTMLHTTPJavaJavaScriptJinja2JSONJuliaKotlinLESSLiquidLuaMarkdownMySQLObjective-COCamlPerlPHPPostgresPowerShellPythonRRubyRustSASSScalaSCSSShellSmartySQLStylusSwiftTOMLTwigTypeScriptXMLYAML

xxxxxxxxxx

34

1

{

2

 "id": "charge\_8108BA4C175O76BD3E06",

3

 "originalId": "charge\_30FDCABC175J76BD3E18",

4

 "liveMode": true,

5

 "created": "2015-04-07T17:55:07Z",

6

 "value": 100,

7

 "currency": "EUR",

8

 "trackId": "TRK2015",

9

 "description": "Sample product",

10

 "responseMessage": "Approved",

11

 "responseAdvancedInfo": "Approved",

12

 "responseCode": "10000",

13

 "status": "Refunded",

14

 "metadata": {

15

 "testKey": "value"

16

 },

17

 "products": \[

18

 {

19

 "name": "Tablet 32gb cellular",

20

 "description": "Tablet 2 gold limited",

21

 "sku": "1aab",

22

 "price": 90,

23

 "quantity": 1,

24

 "image": null,

25

 "shippingCost": 10,

26

 "trackingUrl": "https://www.tracker.com"

27

 }

28

 \],

29

 "udf1": null,

30

 "udf2": null,

31

 "udf3": null,

32

 "udf4": null,

33

 "udf5": null

34

}

Import from Gist

\## Webhook notification Once the charge is successfully refunded, Checkout.com will send a \`charge.refunded\` webhook. 

Thanks for using Checkout.com. If you need any help or support, then message our support team at \[support@checkout.com\](mailto:support@checkout.com).